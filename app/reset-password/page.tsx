...
function ResetInner() {
  ...
  return <AuthShell eyebrow="ACCOUNT RECOVERY" title="Set a new password." description="Use the one-time link from your reset email. Links expire after 60 minutes." reassurance="Reset tokens are single-use; your new password is stored through the existing authentication system.">
    <h2 className="text-lg font-semibold tracking-tight">Reset password</h2>{!token ? <><p role="alert" className="mt-4 rounded-md border border-error/40 bg-error/10 px-3 py-2 text-sm text-error">This reset link is missing or incomplete.</p><p className="mt-5 border-t border-border pt-5 text-sm text-muted-foreground"><Link href="/forgot-password" className="underline underline-offset-4 hover:text-foreground">Request a new link</Link>.</p></> : <form onSubmit={submit} className="mt-6 space-y-4"><div><label htmlFor="password" className="text-sm font-medium">New password</label><input id="password" type="password" required minLength={8} value={password} onChange={(e) => setPassword(e.target.value)} className="field-input mt-1 h-11" autoComplete="new-password" /><p className="mt-1 text-xs text-muted-foreground">At least 8 characters. Never stored in plain text.</p></div>{error && <div role="alert" className="rounded-md border border-error/40 bg-error/10 px-3 py-2 text-sm text-error">{error}</div>}{ok && <div role="status" aria-live="polite" className="rounded-md border border-success/40 bg-success/10 px-3 py-2 text-sm text-success">{ok}</div>}<button type="submit" disabled={loading} className="premium-button premium-button-primary mt-2 w-full">{loading ? "Updating…" : "Update password"}</button></form>}
  </AuthShell>;
}
