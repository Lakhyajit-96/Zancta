...
const faqs = [
  ...
  ["Does OCR run locally?", "Yes. Image OCR uses Tesseract in a browser Worker. English is free. Additional language packs and scanned PDF OCR are Premium. The selected file and recognized text are not sent to an OCR API."],
  ["Can scanned PDFs be converted to text?", "PDF Text Extractor reads existing embedded text from text-native PDFs and reports when a PDF is image-only. Scanned PDF OCR is Local OCR Power on Premium, limited to 20 pages, and runs in the browser."],
  ...
  ["What does Premium provide?", "Premium includes Local OCR Power: Hindi, Bengali, Tamil, Spanish, French, and German language packs, plus scanned PDF OCR up to 20 pages, all in the browser. Other file and page limits match Free. It also reserves an ad-free experience if ads are introduced later. Ads are not live. Premium checkout is available for signed-in, verified accounts when the live payment gate is enabled."],
  ["What happens after cancellation?", "Cancel at period end from Account when Premium is active. You keep Premium until the paid period ends, then access returns to Free. Refunds, invoices, and disputes are handled by Dodo Payments as Merchant of Record. See the refunds and cancellation page."],
  ...
  ["How do I contact support?", "Use the contact form at zancta.tech/contact, or email support@zancta.tech for product questions. Use privacy@zancta.tech for privacy requests, security@zancta.tech for security reports, and billing@zancta.tech for billing. Help, this FAQ, and individual tool pages remain the first self-service guidance. A response-time SLA is not published."],
