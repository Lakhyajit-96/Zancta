  ...
  return (
    ...
    >
      ...
      <ContentSection title="Contact form">
        <p>
          The Contact form collects name, email, topic, subject, message, and an optional account email if you provide one. ZANCTA uses this to route the enquiry to the matching mailbox and to reply. Submissions are sent through the configured production email provider, currently either Hostinger Mail or Resend: an internal notification to ZANCTA and an acknowledgement to you. Contact submissions are not stored as a ZANCTA application database record. The configured email provider may retain copies according to its own practices; ZANCTA does not currently delete those provider copies. Contact submissions are subject to rate limiting that may use IP address and email-derived identifiers for abuse prevention.
        </p>
      </ContentSection>
      <ContentSection title="Transactional email">
        <p>
          Production transactional email is delivered by the configured provider, currently either Hostinger Mail or Resend. Active categories currently include email verification, password reset, password-changed confirmation, welcome, account-deletion confirmation code and deletion notice, contact acknowledgement and internal contact notification, and billing notices (subscription activated, renewed, cancelled, payment failed, and refund processed) when the payment provider sends the corresponding events. A generic security-notification email template exists in the product but is not currently invoked. The configured email provider processes the recipient address and message content for delivery and may retain copies according to its practices.
        </p>
      </ContentSection>
      ...
      <ContentSection title="Retention and deletion">
        ...
        <p>
          {LEGAL_PUBLIC.paymentProviderName} retains customer, payment, subscription, and checkout records as Merchant of Record; ZANCTA cannot delete those provider records. Hostinger Mail, Resend, Google, GitHub, Bing/Microsoft, Vercel, Supabase, and Upstash may retain their own copies according to their practices. ZANCTA cannot necessarily delete those provider-side copies. Google API tokens and snapshots used by the site-operator integration are handled as described in Google API Data — Site Operator Integrations, including on disconnect. Bing Webmaster tokens are handled as described in Bing Webmaster — Site Operator Integrations.
        </p>
      </ContentSection>
